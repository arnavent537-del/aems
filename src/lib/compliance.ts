...
export const COMPLIANCE_FIELDS: ComplianceField[] = [
  ...
  { key: "pfChallanEcrUploadStatus", label: "PF Challan ECR Upload", group: "Challan Upload", statuses: WORKFLOW_STATUSES },
  { key: "esicChallanEcrUploadStatus", label: "ESIC Challan ECR Upload", group: "Challan Upload", statuses: WORKFLOW_STATUSES },
  { key: "ptChallanUploadStatus", label: "PT Challan Upload", group: "Challan Upload", statuses: WORKFLOW_STATUSES },
  { key: "gstChallanUploadStatus", label: "GST Challan Upload", group: "Challan Upload", statuses: WORKFLOW_STATUSES },
  { key: "pfChallanPaidStatus", label: "PF Challan Paid", group: "Challan Paid", statuses: PAID_STATUSES },
  { key: "esicChallanPaidStatus", label: "ESIC Challan Paid", group: "Challan Paid", statuses: PAID_STATUSES },
  { key: "ptPaidStatus", label: "PT Paid", group: "Challan Paid", statuses: PAID_STATUSES },
  { key: "gstPaidStatus", label: "GST Paid", group: "Challan Paid", statuses: PAID_STATUSES },
];

export const COMPLIANCE_STATUS_KEYS = COMPLIANCE_FIELDS.map((f) => f.key);

export function statusColor(value: string): "green" | "blue" | "amber" {
...
export function complianceProgress(record: object): { done: number; total: number } {
  ...
  for (const f of COMPLIANCE_FIELDS) {
    const value = r[f.key];
    if (value === "Completed" || value === "Paid") done += 1;
  }
